...
%Date: 7/3/2023
%--------------------------------------------------------------------------
function [p, cache_cpu , cache_gpu]             = SerialModulationPtychography(p, cache_cpu , cache_gpu)
%--------------------------------------------------------------------------
...
for it                                          = 1:p. recon_iter
    for it2                                     = 1:cache_gpu.num_dps
        ...
        %-------------------------
        %get modulator and dp
        % if it == p. recon_iter - 1
        %     if index_dp == 1
        %         modulator = part_object;
        %         % dp_cmi = cache_gpu. dps_amp(:,:,index_dp).^2;
...
%         cache_gpu.wf_modu_rear_new              = AngularSpectrumPropagation(cache_gpu.wf_ccd_rear , cache_gpu. chirp_backward2, 1);
        cache_gpu.wf_modu_rear_new              = FraunhoferPropagation_backward(cache_gpu.wf_ccd_rear, p. sz_fft,1);
%         cache_gpu.wf_modu_rear_new2              = FresnelPropagation_backward((cache_gpu. wf_ccd_rear), cache_gpu. ChirpFresnel3, cache_gpu. ChirpFresnel4);
%         [cache_gpu.wf_modu_rear_new,temp]    = DFresnelPropagation_backward(cache_gpu.wf_ccd_rear, cache_gpu); 
        ...
        if it                                   > p. pos_refine_after
            ...
            %-------------------------------------
            if p.pos_beta_update_method         == "Tao"
                cache_gpu                       = BetaUpdate(p.pos_beta_update_method,cache_gpu,index_dp);
            end
        end
    end
    if it                                       > p. pos_refine_after
        if p.pos_beta_update_method             == "Zhang"
            cache_gpu                           = BetaUpdate(p.pos_beta_update_method,cache_gpu,index_dp);
        end
    end
    %-------------------------------------
    ...
    if mod(it,p. show_results_every)            ==0
        ...
        cache_cpu.Fig_handels                   = Display('Update',cache_gpu.probe_guess, cache_gpu.object_guess_whole, cache_gpu.modulator_guess, it, p. recon_iter, cache_cpu.Fig_handels);
    end
    %-------------------------------------
end
